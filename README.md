...
## Rural Redesign & Field Operation Guide

To support low-literacy field operators in rural areas of Pakistan, the mobile app has been redesigned with the following features:

1. **Dashboard Simplification**: The multi-tab bottom navigation bar has been replaced with 3 prominent bilingual card buttons for **Crisis Reporting**, **Missing Persons**, and **Aid Distribution**.
2. **Hidden Access Control (Bypass)**: 
   - **Demo Mode**: Rapidly triple-tap the **Shield Icon** 🛡️ in the app bar to reveal the edge-case simulator.
   - **Admin Tools**: Tap the **Settings Gear** ⚙️ to access Agent Traces (AI reasoning log viewer) and Impact comparisons.
3. **Form Safety**: Fields are pre-populated, verified with real-time feedback (invalid CNICs, missing entries), and utilize simple single-tap elements.
4. **Bilingual Typography**: Custom Urdu translated prompts (`فرد مل گیا!`, `رپورٹ بھیجیں`) display alongside English labels to aid field operators.

---

### Step-by-Step Judge Demo Guide

Follow this sequence during your live presentation:

#### 1. The Crisis Flow
- Go to **Report Crisis / مسئلہ بتائیں** 🚨.
- Submit the pre-populated report for Ali Pur.
- Proceed through the stages to view signal extraction (`A1`), source evidence checks (`A2`), severity scoring (`A3`), and dispatch tracking (`A4`).
- Open **Settings ⚙️ -> Agent Traces** to show the judges the precise step-by-step reasoning logs generated by each agent.

#### 2. Missing Person Tracker
- Go to **Missing Person / لاپتہ افراد** 🔍.
- Under **File Report**, register a new case (e.g. `Zainab`, Age `9`, Location `Ali Pur`).
- Submit and see it added live to the active case registry.
- Tap **Possible Match** for `Hassan Ali` at the top of the list, expand details, and tap **Mark as Found** to resolve the match.

#### 3. Aid Distribution & Smart Audits
- Go to **Aid Distribution / امداد کی تقسیم** 📦.
- Enter CNIC `42301-1234567-1` and tap **Verify**. Note the dynamically fetched details for Abdul Rehman.
- Try entering `2` tents and log it. Notice the system flags a fraud risk alert because guidelines limit distributions to `1` tent per household.
- Change the CNIC to `42301-7654321-2` and tap **Verify**. Notice how the registry loads the profile for Fatima Khatoon. Log a valid `1` tent distribution to see the audit check pass.
